...
function movePlatforms() {
    for (let i = 0; i < game.levelGeometry.platformM.length; i++) {
        let plat = game.levelGeometry.platformM[i];

        if (plat.direction === 0) {
            
            if (plat.timer <= 0) {
                if (plat.x === plat.x1 && plat.y === plat.y1)
                    game.levelGeometry.platformM[i].direction = 1;
                if (plat.x === plat.x2 && plat.y === plat.y2)
                    game.levelGeometry.platformM[i].direction = -1;
            }
            
            game.levelGeometry.platformM[i].timer -= 1;
        }
        else {
            ...
            if ((endpointvect[0] * plat.travelvect[0]) + (endpointvect[1] * plat.travelvect[1]) === plat.direction) {
                game.levelGeometry.platformM[i].direction = 0;
                game.levelGeometry.platformM[i].timer = 100;
                game.levelGeometry.platformM[i].x = endpoint[0];
                game.levelGeometry.platformM[i].y = endpoint[1];
            }
        }
        ...
        game.levelGeometry.platformM[i].x += plat.travelvect[0] * plat.direction * plat.speed;
        game.levelGeometry.platformM[i].y += plat.travelvect[1] * plat.direction * plat.speed;
    }
}
...
function checkLandingMoving() {
    for (let i = 0; i < game.levelGeometry.platformM.length; i++) {
        var plat = game.levelGeometry.platformM[i];
...
function drawPlatformsMoving(draw2d) {
